...
var __panel = cc.Node.extend({
	...
	ctor : function () {
		...
		var modelName = "res/tank/test/ld.c3b";
		this.view = new jsb.Sprite3D(modelName);
		var scale = 0.25;
		this.view.setPosition(cc.p(100 + cc.winSize.width/2,cc.winSize.height/2));
	...
	update : function (dt) {
		...
		var _skx = this.skxSlier.value();
		var _sky = this.skySlider.value();
        
		var _px = this.xP.intValue();
		var _py = this.yP.intValue();
        
		this.view.setSkewX(_skx);
		this.view.setSkewY(_sky);
        
		if(this.view.x != _px || this.view.y != _py){
			var pos = this.attachNode.convertToWorldSpace(cc.p(0,0));
			cc.log("attackNode postion x =  " + pos.x +  " , y = " + pos.y);
		}
